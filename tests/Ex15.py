...
class TestUserRegister(BaseCase):
    ...
    def test_user_short_name(self): # с очень коротким именем в один символ
        data = self.prepare_registration_data().copy()
        short_value = 'a'
        data.update({'firstName': short_value})
        response = requests.post("https://playground.learnqa.ru/api/user/", data=data)
        Assertions.assert_code_status(response, 400)
        assert response.content.decode("utf-8") == f"The value of 'firstName' field is short", f"Unexpected response content {response.content}"


    def test_user_with_long_name(self): # с очень длинным именем - длиннее 250 символов
        data = self.prepare_registration_data().copy()
        ...
        assert response.content.decode("utf-8") == f"The value of 'firstName' field is long", f"Unexpected response content {response.content}"
